...
```{r target-rule-repeat, echo=FALSE}
att <- function(n=1e5, p=0.3, d=1){
  ...
  df$treat2 = ifelse(df$target2==1 & df$control2==0, 1, 0)
  df$delta2 = ifelse(df$treat2==1 & df$type==1 & df$delta==0 & runif(n, 0, 1)<=p, d, 0)
  df$spend3 = df$delta2 + df$spend2

  highnt = sum(df$spend2>4 & df$type==1 & df$delta==0 & df$control2==0)
  hight = sum(df$spend2>4 & df$type==1 & df$delta==1 & df$control2==0)
  low = sum(df$spend2>4 & df$type==0 & df$control2==0)
  prop_treat = highnt / (highnt + hight + low)

  ### A lot more high non-treated in the control group
  highnt = sum(df$spend2>4 & df$type==1 & df$delta==0 & df$control2==1)
  hight = sum(df$spend2>4 & df$type==1 & df$delta==1 & df$control2==1)
  low = sum(df$spend2>4 & df$type==0 & df$control2==1)
  prop_control = highnt / (highnt + hight + low)

  ### Average treatment effect 2 (Less than predicted!)
  df_subset2 = df[df$target2==1,]
  mod <- lm(spend3 ~ treat2, data=df_subset2)
  return(list(prop_treat, prop_control, coef(mod)["treat2"]))
}

B = 500
prop_treat_list = list()
prop_control_list = list()
coef_list = list()
for(b in 1:B){
  temp <- att()
  prop_treat_list[[b]] <- temp[[1]]
  prop_control_list[[b]] <- temp[[2]]
  coef_list[[b]] <- temp[[3]]
}
```

```{r target-rule-results}
mean(unlist(coef_list))
