...
bibliography: [benchmark-of-popular-graph-network-packages.bib] 
link-citations: true
categories: []
...
- Lightgraphs v2.0 is included in the benchmark exercise.^[Initial results were committed to the repository in the beginning of the year, but this revised benchmark uses v2.0 release of lightgraphs.] It is the first Julia library to be added to the study - read on to find out how it fares with the rest.
...
- Graph-tool performance comparison [@graphtool2], compares graph-tool with igraph and networkx on the single-source shortest path, page rank, k-core, minimum spanning tree and betweeness algorithms.
...
networkit:
```python
distance.BFS(g, node_index).run().getDistances(False)
```
...
lightgraphs:
```julia
shortest_paths(g, 1, BFS(RadixSort)) # Non-threaded
shortest_paths(g, 1, ThreadedBFS())  # Threaded
```
