...
## Twitter Project `r emo::ji("bird")`

![](/img/serverless/sentiment1.PNG)
...
![](/img/serverless/twitter_api.PNG)
...
That's it for the R code. Now you should have a working application which you can run locally from your computer. Next, we get into the grimy details of ML-ops `r emo::ji("exercise")`. This involves packaging our dependencies with docker and deploying it on cloud run.

## Docker

Docker is a platform which packages different software, configurations and environments into containers which neatly encapsulate your application. The end user just needs to list out the installation steps to build the image which can subsequently be run on the docker platform `r emo::ji("whale")`. 
...
![](/img/serverless/cloud_run_success.PNG)
...
The most followed person on Twitter with 112 million followers does not actually tweet too frequently `r emo::ji("thinking")`:  
https://twitter-r-cvdvxo3vga-uc.a.run.app/frequency?n=500&users=BarackObama

![](/img/serverless/freq_obama_tweets.PNG)
...
![](/img/serverless/sentiment2.PNG)

## Conclusion

That's it for this serverless + R tutorial. Hope you managed to learn something useful or at the very least find the twitter analysis interesting `r emo::ji("smiley")`. The nice part about having this application running is that you can analyze the real-time twitter frequency or sentiment plots with whatever account you choose (even your own).^[I will keep it up and running as long as I can as long as the usage does not suddenly spike up and exceed the free tier limit]
