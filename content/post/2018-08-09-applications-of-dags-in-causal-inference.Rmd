...
author: Timothy Lin
date: '2018-08-09'
slug: applications-of-dags-in-causal-inference
categories: []
...
library(ggdag)
knitr::opts_chunk$set(warning=FALSE, message=FALSE, cache=TRUE, out.width='60%', out.height = '50%')
```
...
__Graphs, paths and d-seperation__  
...
```{r mgraph, echo=FALSE}
coords <- list(
  x = c(X = 0, Y = 2, Z = 1, A = 3, E = -1),
  y = c(X = 1, Y = 1, Z = 1.35, A = 1.5, E = 1.5))
...
```{r levittsyverson1, echo=FALSE}
coords <- list(
  x = c(X = 0, Y = 2, Q = 1, K = -1, C = 3),
  y = c(X = 1, Y = 1, Q = 1.5, K = 1.5, C = 1.5))

dagify(X ~ C + Q + K,
       Y ~ X + C + Q + K, coords=coords) %>% 
  ggdag(text_size = 5) + theme_dag()
```
...
```{r levittsyverson2, echo=FALSE}
coords <- list(
  x = c(X = 0, Y = 2, Q = 1, K = -1, C = 3, U=-2),
  y = c(X = 1, Y = 1, Q = 1.5, K = 1.5, C = 1.5, U=1.3))
...
```{r rainfall, echo=FALSE}
coords <- list(
  x = c(X = 0, Y = 2, U = 1, Z = -1, C= 0.5),
  y = c(X = 1, Y = 1, U = 1.5, Z = 1, C = 0.5))
dagify(X ~ U + Z,
       C ~ Z,
       Y ~ X + C + U, coords=coords) %>% 
  ggdag(text_size = 5) + theme_dag()
```
