...
Now we want to find the value $p$ that maximises the likelihood, $L(p)$. One can alternatively maximise the log likelihood which makes the derivation easier.^[Since the $ln$ function is monotonic, the parameter value that maximises the log likelihood will also maximise the likelihood.] The maximum likelihood estimate can then be calculated by finding the value $p$ that maximises the log likelihood:
...
Similarly, one can derive the formula for the OLS estimator through the maximum likelihood approach. Recall that the linearity implies the following specification for the regression model: $y_{i} = \mathbf{x}_{i}'\beta + u_{i}$. In the maximum likelihood approach, we need to assume that the error terms conditional on $x_{i}$ are normally distributed with unknown variance i.e. $ u_{i} | \mathbf{x}_{i} \sim N(0,\sigma^2)$. The PDF of a single observation is given by:
$$
...
In most cases, such as the logit model discussed above, there is no explicit solution for the maximisation problem and the solution has to be derived using numerical optimisation.