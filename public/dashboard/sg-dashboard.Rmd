...
plotly_build(sector.bar) %>% layout(margin = list(l = 150))
...
ggplotly(overall_emp, tooltip=c("y","x")) %>% layout(margin = list(b = 80))
```
...
ggplotly(pct_change_emp, tooltip=c("period","pct_change_emp", "industry")) %>% layout(margin = list(b = 80), legend = list(orientation = 'h',xanchor = "center", yanchor="bottom", y=-0.5))
```
...
ggplotly(change_emp_year_plot) %>% layout(margin = list(l = 150))
```
